...
class App(CTk):
    def __init__(self):
        ...
        self.title('Sirok')
        '''Menu'''
        self.menu_frame = CTkFrame(self, fg_color="#18ddd7",width=30,height=400)
        self.menu_frame.pack_propagate(False)
        self.menu_frame.place(x=0,y=0)
        self.btn_menu = CTkButton(self.menu_frame,text=">",font=("Helvetica",20,"bold"), command=self.toggl_menu)
        self.btn_menu.place(x=0,y=0,relwidth=1)
        """scroll frame"""
        self.scroll_frame = CTkScrollableFrame(self,fg_color='#21d150')
    ...
    def save_name(self):
        if self.name_entry.get():
            ...
            try:
                ...
                self.add_recv_msg("Сервер не знайдено")
                self.sock.close()
    ...
    def adaptive_ui(self):
        ...
        self.btn_send.place(x=self.msg_text_entry.winfo_width() + self.menu_frame.winfo_width()+5,y=self.winfo_height() -50)
    ...
    def add_send_msg(self,text):
        msg_frame = CTkFrame(self.scroll_frame,fg_color="white")
        msg_frame.pack(pady=10,padx=10,anchor="e")
        wrap_width = self.winfo_width() -self.menu_frame.winfo_width() -40
